...
public class FilmDAO {
    ...
    public void addFilm(Film film) {
        try (Connection connection = DBConnection.getConnection(); PreparedStatement statement = connection.prepareStatement("INSERT INTO films (title, category, review) VALUES (?, ?, ?)")) {
            statement.setString(1, film.getTitle());
            statement.setString(2, film.getCategory());
            statement.setString(3, film.getReview());
    ...
    public void updateFilm(Film film) {
        try (Connection connection = DBConnection.getConnection(); PreparedStatement statement = connection.prepareStatement("UPDATE films SET title = ?, category = ?, review = ? WHERE id = ?")) {
            statement.setString(1, film.getTitle());
            statement.setString(2, film.getCategory());
            statement.setString(3, film.getReview());
